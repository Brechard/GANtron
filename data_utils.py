...
class TextMelLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, audiopaths_and_text, hparams, wavs_path):
        ...
        if hparams.vesus_path:
            ...
            # ids = list(range(len(audiopaths_and_text)))
            ...
            # text_train = ''
            # for pos in ids[:-250]:
            #     text_train += '/'.join(audiopaths_and_text[pos][0].split('/')[-3:])
            #     text_train += '|' + audiopaths_and_text[pos][1]
            #     text_train += '|' + str(speakers[pos])
            #     text_train += '|' + str(emotions[pos])[1:-1] + '\n'
            # with open('filelists/vesus_train.txt', 'w', encoding='UTF-8') as file:
            #     file.write(text_train)
            # text_val = ''
            # for pos in ids[-250:-200]:
            #     text_val += '/'.join(audiopaths_and_text[pos][0].split('/')[-3:])
            #     text_val += '|' + audiopaths_and_text[pos][1]
            #     text_val += '|' + str(speakers[pos])
            #     text_val += '|' + str(emotions[pos])[1:-1] + '\n'
            # with open('filelists/vesus_val.txt', 'w', encoding='UTF-8') as file:
            #     file.write(text_val)
            # text_test = ''
            # for pos in ids[-200:]:
            #     text_test += '/'.join(audiopaths_and_text[pos][0].split('/')[-3:])
            #     text_test += '|' + audiopaths_and_text[pos][1]
            #     text_test += '|' + str(speakers[pos])
            #     text_test += '|' + str(emotions[pos])[1:-1] + '\n'
            # with open('filelists/vesus_test.txt', 'w', encoding='UTF-8') as file:
